// Vercel serverless proxy for The Odds API — The Open Championship outright winner odds.
...
const UPSTREAM = 'https://api.the-odds-api.com/v4/sports/golf_the_open_championship_winner/odds';

const PLAY_HOURS_TTL_MS = 15 * 60 * 1000;
const OFF_HOURS_TTL_MS = 4 * 60 * 60 * 1000;

// Tournament play window in America/New_York time.
// Thu 7/16 -> Sun 7/19, inclusive. 06:00 - 21:00 ET each day.
...
function normalize(upstreamJson) {
  ...
  // The Open Championship outright is a single "event"; if multiple are returned we merge.
...
export default async function handler(req, res) {
  ...
  const now = Date.now();
  const cache = globalThis.__BRITISH_OPEN_ODDS_CACHE;
  ...
  try {
    ...
    globalThis.__BRITISH_OPEN_ODDS_CACHE = { fetchedAtMs: now, payload };
