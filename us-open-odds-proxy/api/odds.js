// Vercel serverless proxy for The Odds API — U.S. Open outright winner odds.
...
const UPSTREAM = 'https://api.the-odds-api.com/v4/sports/golf_us_open_winner/odds';

const PLAY_HOURS_TTL_MS = 15 * 60 * 1000;
const OFF_HOURS_TTL_MS = 4 * 60 * 60 * 1000;

// Tournament play window in America/New_York time.
// Thu 6/18 -> Sun 6/21, inclusive. 06:00 - 21:00 ET each day.
...
function normalize(upstreamJson) {
  ...
  // The U.S. Open outright is a single "event"; if multiple are returned we merge.
...
export default async function handler(req, res) {
  ...
  const now = Date.now();
  const cache = globalThis.__US_OPEN_ODDS_CACHE;
  ...
  try {
    ...
    globalThis.__US_OPEN_ODDS_CACHE = { fetchedAtMs: now, payload };
